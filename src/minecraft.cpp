...
namespace minecraft {
    ...
    int Minecraft::initialise() {
        if(glfwInit() != GL_TRUE){
            std::cerr << "GLFW initialisation failed.\n";
            return 1;
        }
        
        // Minecraft already uses OpenGL but whatever
        this->window = glfwCreateWindow(window_width, window_height, "Minecraft: OpenGL (tm) version", nullptr, nullptr);
        if(!window){
        ...
        auto error_callback = [](int error, const char* msg) {
            print_error(std::string("OpenGL error ") + std::to_string(error) + " (" + from_GL_error(error) + "): " + msg);
        };
        ...
        Shader vertex(GL_VERTEX_SHADER, slurp(std::ifstream("assets/shaders/main.vsh")));
        vertex.compile();
        if(vertex.has_error()) {
            print_error(std::string("Failed to compile vertex shader: ") + vertex.get_error());
            glfwTerminate();
            return 1;
        }

        Shader fragment(GL_FRAGMENT_SHADER, slurp(std::ifstream("assets/shaders/main.fsh")));
        fragment.compile();
        if(fragment.has_error()) {
            print_error(std::string("Failed to compile fragment shader: ") + fragment.get_error());
            glfwTerminate();
            return 1;
        }
        ...
        Texture terrain("assets/textures/terrain.png");
        ...
        projection <<
            1 / (window_width * 1.0f / window_height * std::tan(fov / 2)), 0, 0, 0,
